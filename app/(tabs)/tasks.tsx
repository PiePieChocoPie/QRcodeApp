                ...
                >
                    <View style={[styles.expanderContainer, { backgroundColor: "transparent", marginTop: '10%' }]}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                            <Pressable onPress={() => focusChange('Tasks')} style={[styles.listElementContainer, { width: '40%', alignItems: "center", flex: 1 }, selectedList === 'Tasks' ? { backgroundColor: projColors.currentVerse.extra } : { backgroundColor: projColors.currentVerse.extrasecond }]}>
                            ...
                            <Pressable onPress={() => focusChange('Attorney')} style={[styles.listElementContainer, { width: '40%', alignItems: "center", flex: 1 }, selectedList === 'Attorney' ? { backgroundColor: projColors.currentVerse.extra } : { backgroundColor: projColors.currentVerse.extrasecond }]}>
                            ...
                            renderItem={({ item }) =>
                                <Pressable onPress={() => setFilterIndex(item.id)}>
                                    <View style={[
                                        ...
                                        { backgroundColor: filterIndex === item.id ? item.colors[0] : '#FFFFFF' }, // Установка цвета по умолчанию, например, белый
                                        { padding: 15, margin: 7 }
                                    ]}>
                                        <Text style={[
                                            styles.Title,
                                            { color: filterIndex === item.id ? item.colors[1] : '#000000' } // Установка цвета текста по умолчанию, например, черный
                                        ]}>
                                            {item.status}
                                        </Text>
                                    </View>
